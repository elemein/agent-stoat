...
Local LLMs output tool calls in many formats — native function-calling, XML tags,
markdown code blocks, or raw JSON. This module tries each format in order of
reliability (native > XML > markdown > raw JSON) and returns a normalized list.

Also handles common LLM quirks: <think> blocks, trailing commas, unquoted keys.
"""
...
def extract_tool_calls(response: dict) -> list[dict]:
    ...
    1. Native tool_calls field (OpenAI-compatible format)
...
def _parse_native_tool_calls(tool_calls: list) -> list[dict]:
    """Parse native tool_calls format (OpenAI-compatible)."""
    result = []
...
def _extract_json_object(content: str, start: int) -> str | None:
